...
class BuildTree:
    ...
    def simulation(self, input_node:"Node", stock: list, path_to_store_anions: "open") -> dict:
        """
        Simulate a random rollout from the selected leaf node

        :param input_node: the input node to run rollout
        :stock: a stock of existing anions, which will be used to check the novelty of the SMILES
        :file_to_store_anions: a file to save generated anions
        ...
        for child in input_node.children:
            ...
            print('generate anions %s'%node_SMILES_dict[child])
        score_dict, anions = reward_score(node_SMILES_dict, stock)
        with open(path_to_store_anions, 'a') as file1:
            for anion in anions:
                print(anion, file=file1)
        return score_dict
    ...
    def search(self, stock_path: str, 
               saved_anions_path: str, 
               num_loops: int,
               root_SMILES: list = ['^']) -> None:
        """
        To run the tree search.

        :param stock_path: a path to the stock of existing anions, which will be used to check the novelty of the SMILES
        :param saved_anions_path: a path to save generated anions
        ...
        for _ in range(num_loops):
            ...
            score_dict = self.simulation(node, stock, saved_anions_path)
    ...
    for node in node_SMILES_dict:
        ...
        for item in smiles_checking_result:
            if item != 'NaN':
                anions.append(item)
                score_dict[node] = 1
    return score_dict, list(set(anions))
...
def check_smiles(input_SMILES: str, stock: list) -> str:
    ...
    if mol != None:
        ...
        if canonical_smiles.count('-]') == 1 and canonical_smiles.count('+]') == 0 \
           ...
           and canonical_smiles.count('+2]') == 0 and canonical_smiles.count('+3]') == 0:
            print('find anions %s'%input_SMILES)
